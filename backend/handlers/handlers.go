...
func (h *H) RequestLoan(c *gin.Context) {
	...
	// Edge case: degraded score — refuse loan, data is incomplete
	if score.Degraded {
	...
	// ── Step 3: Validate amount vs tier ──────────────────────────────────────
	...
	// ── Step 5: Relay proof via Starknet → EVM ─────────────────────
	snResult, err := h.Starknet.ValidateCredit(wallet, score.Score, req.AmountUSDC)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to validate credit via Starknet — please try again",
		})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to relay proof to EVM — please try again",
		})
		return
	}
	...
	// Atomic check + create — fixes check-then-act race condition
	if err := h.Store.CreateLoanIfNoActive(loan); err != nil {
		c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		return
	}

	// ── Step 8: Async — submit to chain + store on Filecoin ──────────────────
	// These run after the loan is already saved in-memory.
	// They don't block the HTTP response.
	// If they fail, the loan still exists in-memory and the tx hash
	// gets updated when it eventually succeeds.
	go func() {
		defer func() {
			if r := recover(); r != nil {
	...
	c.JSON(http.StatusOK, models.LoanRequestResponse{
		...
		Tier:             score.Tier,
		OnChainConfirmed: false, // relay tx was broadcast; requestLoan() by user is still pending
...
func (h *H) RepayLoan(c *gin.Context) {
	...
	// Atomic repay — fixes check-then-update race condition
	...
	// Submit repayment on-chain + notify async
	go func() {
		defer func() {
			if r := recover(); r != nil {
...
func (h *H) GetTiers(c *gin.Context) {
	// Proxy to Python — single source of truth for tier definitions
